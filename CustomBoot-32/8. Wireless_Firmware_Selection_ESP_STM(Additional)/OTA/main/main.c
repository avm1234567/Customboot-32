...
#define FIRMWARE_GPIO GPIO_NUM_18
...
esp_err_t FirmwareA_Handler(httpd_req_t *req)
{
    gpio_set_level(FIRMWARE_GPIO, 0);
    httpd_resp_send(req, "Execute firmware A", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

/* Handler for LED OFF */
esp_err_t FirmwareB_Handler(httpd_req_t *req)
{
    gpio_set_level(FIRMWARE_GPIO, 1);
    httpd_resp_send(req, "Execute firmware B", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}

esp_err_t STM_RESET_Handler(httpd_req_t *req)
{
...
static httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(server, &(httpd_uri_t){
            .uri = "/Firmware/A",
            .method = HTTP_GET,
            .handler = FirmwareA_Handler,
            .user_ctx = NULL});

        httpd_register_uri_handler(server, &(httpd_uri_t){
            .uri = "/Firmware/B",
            .method = HTTP_GET,
            .handler = FirmwareB_Handler,
            .user_ctx = NULL});
        
        httpd_register_uri_handler(server, &(httpd_uri_t){
            .uri = "/Reset",
            .method = HTTP_GET,
            .handler = STM_RESET_Handler,
            .user_ctx = NULL});
    }
...
{
    gpio_reset_pin(FIRMWARE_GPIO);
    gpio_set_direction(FIRMWARE_GPIO, GPIO_MODE_OUTPUT);
    gpio_set_level(FIRMWARE_GPIO, 0);
