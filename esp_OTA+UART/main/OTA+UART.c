...
static esp_err_t boot_new_app(const char *filepath) {
    ...
    if (!f) {
        ESP_LOGE("OTA", "Failed to open file: %s", filepath);
        return ESP_FAIL;
    }
    ...
    return ESP_OK; // This line is technically unreachable, but safe
}
...
if (dot && strcmp(dot, ".bin") == 0) {
    ESP_LOGI(TAG, "BIN file uploaded. Starting OTA...");
    boot_new_app(filepath);
}
